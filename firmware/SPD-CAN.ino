...
void processSpeeduinoData()
{
  ...
  int mapFc  = map * 10;   // Nano em kPa

  int ectFc  = ect * 10;

  int battFc = batt * 10;

  float pwFc = dutyCycle * 10;
  ...
  addMeasure(payload, idx, 0x0084, rpmFc);   // RPM

  addMeasure(payload, idx, 0x008E, advFc);

  addMeasure(payload, idx, 0x0002, tpsFc);   // TPS

  addMeasure(payload, idx, 0x0004, mapFc);   // MAP

  addMeasure(payload, idx, 0x0008, ectFc);   // ECT

  addMeasure(payload, idx, 0x0012, battFc);  // BAT

  addMeasure(payload, idx, 0x008A, pwFc);    // Inj. A
